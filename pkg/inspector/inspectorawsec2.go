...
func NewEC2Scanner(regions []string) (Inspector, error) {
...
func (s *EC2Scanner) Scan(ctx context.Context, config configuration.TaggyScanConfig) (*ScanResult, error) {
	result := &ScanResult{
		StartTime: time.Now(),
		Region:    s.Regions[0], // Using first region for now
	}
